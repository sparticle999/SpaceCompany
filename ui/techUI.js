...
Templates.techUI = function(cPage, cTitle, cObj) {
	...
	var TemplatePaneNonMachine = Handlebars.compile(
		['<tr id="{{htmlId}}" {{#if unlocked}}class=""{{else}}class="hidden"{{/if}}>',
		   '<td>',
		     '<h3 id="{{htmlIdTitle}}" class="default btn-link">{{name}}: <span id="{{htmlId}}_Count">0</span></h3>',
		     '<span>',
		       '{{desc}}',
		       '<br>',
		       '<p id="{{htmlIdCost}}"></p>',
		     '</span>',
			 '<button type="button" id="{{htmlIdButton}}" class="btn btn-default">{{buttonText}}</button>',
		   '</td>',
		 '</tr>',''].join('\n'));
	 ...
	 * @return {string}              generated html containing all buildings producing a resource.
	 */
	var buildNonMachineCost = function(buildingData, prefix) {
		var html = ""
		Object.keys(buildingData).forEach(function(build) {
	...
	var createPane = function(data) {
		...
		// The item is something else.  Do we need a buy button?
			...
			attachHTML(cPage+'Tab_'+t.item+'_netc', buildNonMachineCost(data.items,''))
		}
		return "";
	}
